...
describe('search engine', () => {
  it('always returns a legal move from the starting position', () => {
    ...
    const result = findBestMove(chess, { maxDepth: 2, timeBudgetMs: 1000 });
  ...
  it('does not mutate the board it was given (move/undo balance)', () => {
    const chess = new Chess();
    const fenBefore = chess.fen();
    findBestMove(chess, { maxDepth: 2, timeBudgetMs: 1000 });
    expect(chess.fen()).toBe(fenBefore);
  });

  it('finds a mate-in-1 when one is available', () => {
    ...
    const result = findBestMove(mateInOne, { maxDepth: 3, timeBudgetMs: 1500 });
  ...
  it('prefers not to lose material for nothing (avoids hanging the queen)', () => {
    ...
    const result = findBestMove(chess, { maxDepth: 3, timeBudgetMs: 1500 });
    expect(`${result.bestMove.from}${result.bestMove.to}`).not.toBe('d1d7');
  });

  it('respects the configured search depth ceiling (does not throw for depth 1)', () => {
    const chess = new Chess();
    const result = findBestMove(chess, { maxDepth: 1, timeBudgetMs: 500 });
    expect(result.depthReached).toBeGreaterThanOrEqual(1);
  });

  it('throws when given a position with no legal moves', () => {
    const checkmated = new Chess('6k1/6Q1/6K1/8/8/8/8/8 b - - 0 1');
    expect(checkmated.isCheckmate()).toBe(true);
    expect(() => findBestMove(checkmated, { maxDepth: 2, timeBudgetMs: 500 })).toThrow();
  });
});
