...
import { SpellShopModal } from '@/components/SpellShopModal';
...
import { applyExplosion, applyTeleport } from '@/engine/spellEffects';
...
import { CAPTURE_GOLD_VALUE, CHECK_GOLD_REWARD, getSpellDef, type OwnedSpell, type SpellId } from '@/domain/spells';
...
function findKingSquare(chess: Chess, color: PieceColor): Square | null {
...
function emptyOwnedCounts(): Record<SpellId, number> {
  return { explosion: 0, teleport: 0, shield: 0, leap: 0 };
}
...
export default function GameScreen() {
  ...
  const isPlayerTurn = !isGameOver && derivedChess.turn() === resolvedPlayerColor && !promotionPending && !isAiThinking;
  ...
  const ownedCounts = useMemo(() => {
    ...
    for (const spell of ownedSpells) counts[spell.spellId] += 1;
    return counts;
  }, [ownedSpells]);

  // Squares highlighted while a spell is armed, replacing normal move highlighting.
  const spellHighlightTargets = useMemo(() => {
    if (!armedSpell) return [];
    const squares: Square[] = [];
    for (const row of derivedChess.board()) {
      for (const cell of row) {
        if (!cell || cell.color !== resolvedPlayerColor) continue;
        if (armedSpell === 'explosion' && cell.type !== 'p') continue;
        if (armedSpell !== 'explosion' && cell.type === 'k') continue;
        if (spellCastTargets.includes(cell.square)) continue;
        squares.push(cell.square);
      }
    }
    return squares;
  }, [armedSpell, derivedChess, resolvedPlayerColor, spellCastTargets]);
  ...
  /** Shared bookkeeping after ANY move completes (normal chess.js move or a leap). */
  const finishMoveEffects = useCallback(
    (params: { moverColor: PieceColor; from: Square; to: Square; capturedType: PieceSymbol | null; isPromotion: boolean }) => {
      ...
      // The leap buff only ever lasts for the single move right after it's cast — once that
      // side's turn ends (whether or not the leaping piece was the one moved), it expires.
      if (params.moverColor === resolvedPlayerColor) setLeapArmedSquare(null);
      ...
      if (params.moverColor === resolvedPlayerColor) {
        ...
        if (isCheck && !isMate) goldGained += CHECK_GOLD_REWARD;
        if (goldGained > 0) {
          setGold((g) => g + goldGained);
          playSfx('gold');
        }
        // The shield now guards the opponent's very next turn only.
        if (shieldedSquare) {
          setShieldedSquare((current) => (current === params.from ? params.to : current));
          setShieldArmedForAiTurn(true);
        }
      } else if (shieldArmedForAiTurn) {
        // The opponent's turn just resolved — the shield's protection window is over.
    ...
    [timeControl, playSfx, haptics, finalizeGame, resolvedPlayerColor, shieldedSquare, shieldArmedForAiTurn],
  );
  ...
  const handleSpellTargetTap = (square: Square) => {
    ...
    if (armedSpell === 'explosion') {
      ...
      if (shieldedSquare && destroyedSquares.includes(shieldedSquare)) {
        setShieldedSquare(null);
        setShieldArmedForAiTurn(false);
      }
      if (leapArmedSquare && destroyedSquares.includes(leapArmedSquare)) setLeapArmedSquare(null);
      consumeArmedSpell();
      setFen(chess.fen());
  ...
  const handlePieceDrop = (from: Square, to: Square) => {
    ...
    const targets = isLeapMove ? getLeapDestinations(chess, from) : chess.moves({ square: from, verbose: true }).map((m) => m.to);
  ...
  const boardSize = useMemo(() => Math.min(Dimensions.get('window').width - spacing.md * 2, 420), []);
        ...
        <View style={styles.center}>
          ...
          {armedSpell && (
            <Text style={styles.statusText}>
              ...
              {(armedSpell === 'shield' || armedSpell === 'leap') && t('spell.chooseAllyTarget')}
            </Text>
          )}
